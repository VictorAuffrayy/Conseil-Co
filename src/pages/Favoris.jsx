...
function WatchView({ watch, onEdit, onDelete, favorites, onToggleFav }) {
  ...
  const load = async () => {
    ...
    const cutoff = Date.now() - 72 * 60 * 60 * 1000
    const recent = unique.filter(a => !a.pubDate || new Date(a.pubDate).getTime() > cutoff)
    // Fallback : si rien sur les 72h, on prend tous les articles disponibles
    // triés par pertinence/date, peu importe leur âge.
    const pool = recent.length > 0 ? recent : unique
    setUsingFallback(recent.length === 0 && unique.length > 0)
    const scored = pool
      .map(a => ({ ...a, relevanceScore: scoreArticle(a, watch.keywords || []), watchId: watch.id, watchName: watch.name, watchColor: watch.color }))
      .sort((a, b) => {
        // Tri principal par pertinence, puis par date décroissante en cas d'égalité
        if (b.relevanceScore !== a.relevanceScore) return b.relevanceScore - a.relevanceScore
        if (!a.pubDate && !b.pubDate) return 0
        if (!a.pubDate) return 1
        if (!b.pubDate) return -1
      ...
      <div className="mvi-watch-header" style={{ borderLeftColor: watch.color }}>
        <div className="mvi-watch-header-left">
          ...
          <div className="mvi-watch-meta">
            ...
            {usingFallback && (
              <span className="mvi-fallback-badge" title="Aucun article récent (moins de 72h) — affichage des articles les plus pertinents disponibles, toutes dates confondues.">
                Aucun article récent · les plus pertinents affichés
              </span>
            )}
          </div>
        </div>
...
const css = `
  ...
  .mvi-watch-meta { display: flex; align-items: center; gap: 6px; font-size: 0.76rem; color: var(--color-text-muted); flex-wrap: wrap; }
  .mvi-fallback-badge { background: #FFFBEB; border: 1px solid #FDE68A; color: #92400E; font-size: 0.7rem; font-weight: 500; padding: 2px 9px; border-radius: 10px; margin-left: 4px; }
